...
def fix_injury_mode
  return unless Script.current.session.xml_data.injury_mode == 2
  Script.current.session._puts '_injury 2'
  150.times { sleep 0.05; break if Session.current.xml_data.injury_mode == 2 }
end
...
def waitrt
  wait_until { (Session.current.xml_data.roundtime_end.to_f - Time.now.to_f + Session.current.xml_data.server_time_offset.to_f) > 0 }
  sleep((Session.current.xml_data.roundtime_end.to_f - Time.now.to_f + Session.current.xml_data.server_time_offset.to_f + 0.6).abs)
end

def waitrt?
  rt = Session.current.xml_data.roundtime_end.to_f - Time.now.to_f + Session.current.xml_data.server_time_offset.to_f + 0.6
  sleep rt if rt > 0
end

def waitcastrt
  wait_until { (Session.current.xml_data.cast_roundtime_end.to_f - Time.now.to_f + Session.current.xml_data.server_time_offset.to_f) > 0 }
  sleep((Session.current.xml_data.cast_roundtime_end.to_f - Time.now.to_f + Session.current.xml_data.server_time_offset.to_f + 0.6).abs)
end

def waitcastrt?
  rt = Session.current.xml_data.cast_roundtime_end.to_f - Time.now.to_f + Session.current.xml_data.server_time_offset.to_f + 0.6
  sleep rt if rt > 0
end

def checkrt
  [Session.current.xml_data.roundtime_end.to_f - Time.now.to_f + Session.current.xml_data.server_time_offset.to_f + 0.6, 0].max
end

def checkcastrt
  [Session.current.xml_data.cast_roundtime_end.to_f - Time.now.to_f + Session.current.xml_data.server_time_offset.to_f + 0.6, 0].max
end

def checkpoison
  Session.current.xml_data.indicator['IconPOISONED'] == 'y'
end

def checkdisease
  Session.current.xml_data.indicator['IconDISEASED'] == 'y'
end

def checksitting
  Session.current.xml_data.indicator['IconSITTING'] == 'y'
end

def checkkneeling
  Session.current.xml_data.indicator['IconKNEELING'] == 'y'
end

def checkstunned
  Session.current.xml_data.indicator['IconSTUNNED'] == 'y'
end

def checkbleeding
  Session.current.xml_data.indicator['IconBLEEDING'] == 'y'
end

def checkgrouped
  Session.current.xml_data.indicator['IconJOINED'] == 'y'
end

def checkdead
  Session.current.xml_data.indicator['IconDEAD'] == 'y'
end
...
def checkhidden
  Session.current.xml_data.indicator['IconHIDDEN'] == 'y'
end

def checkinvisible
  Session.current.xml_data.indicator['IconINVISIBLE'] == 'y'
end

def checkwebbed
  Session.current.xml_data.indicator['IconWEBBED'] == 'y'
end

def checkprone
  Session.current.xml_data.indicator['IconPRONE'] == 'y'
end

def checknotstanding
  Session.current.xml_data.indicator['IconSTANDING'] == 'n'
end

def checkstanding
  Session.current.xml_data.indicator['IconSTANDING'] == 'y'
end
...
def checkname(*strings)
  ...
  if strings.empty?
      ...
      Session.current.xml_data.name =~ /^(?:#{strings.join('|')})/i
  end
end
...
def move(dir='none', giveup_seconds=30, giveup_lines=30)
  ...
  line_count = 0
  room_count = Session.current.xml_data.room_count
  giveup_time = Time.now.to_i + giveup_seconds.to_i
  save_stream = Array.new

  put_dir = proc {
      if Session.current.xml_data.room_count > room_count
...
def checkpaths(dir="none")
  if dir == "none"
      if Session.current.xml_data.room_exits.empty?
        return false
      else
        return Session.current.xml_data.room_exits.collect { |dir| dir = SHORTDIR[dir] }
      end
  else
      Session.current.xml_data.room_exits.include?(dir) || Session.current.xml_data.room_exits.include?(SHORTDIR[dir])
  end
end
...
def check_mind(string=nil)
  if string.nil?
      return Session.current.xml_data.mind_text
  elsif (string.class == String) and (string.to_i == 0)
      if string =~ /#{Session.current.xml_data.mind_text}/i
        return true
      else
        return false
      end
  elsif string.to_i.between?(0,100)
      return string.to_i <= Session.current.xml_data.mind_value.to_i
  else
...
def checkmind(string=nil)
  if string.nil?
      ...
      if mind_state.index(Session.current.xml_data.mind_text)
        mind = mind_state.index(Session.current.xml_data.mind_text) + 1
...
def percentmind(num=nil)
  if num.nil?
      Session.current.xml_data.mind_value
  else
      Session.current.xml_data.mind_value >= num.to_i
  end
end

def checkfried
  if Session.current.xml_data.mind_text =~ /must rest|saturated/
      true
  else
      false
  end
...
def checksaturated
  if Session.current.xml_data.mind_text =~ /saturated/
      true
  else
      false
  end
...
def checkmana(num=nil)
  if num.nil?
      Session.current.xml_data.mana
  else
      Session.current.xml_data.mana >= num.to_i
  end
end

def maxmana
  Session.current.xml_data.max_mana
end

def percentmana(num=nil)
  if Session.current.xml_data.max_mana == 0
      percent = 100
  else
      percent = ((Session.current.xml_data.mana.to_f / Session.current.xml_data.max_mana.to_f) * 100).to_i
  end
  if num.nil?
      percent
  else
...
def checkhealth(num=nil)
  if num.nil?
      Session.current.xml_data.health
  else
      Session.current.xml_data.health >= num.to_i
  end
end

def maxhealth
  Session.current.xml_data.max_health
end

def percenthealth(num=nil)
  if num.nil?
      ((Session.current.xml_data.health.to_f / Session.current.xml_data.max_health.to_f) * 100).to_i
  else
      ((Session.current.xml_data.health.to_f / Session.current.xml_data.max_health.to_f) * 100).to_i >= num.to_i
  end
end

def checkspirit(num=nil)
  if num.nil?
      Session.current.xml_data.spirit
  else
      Session.current.xml_data.spirit >= num.to_i
  end
end

def maxspirit
  Session.current.xml_data.max_spirit
end

def percentspirit(num=nil)
  if num.nil?
      ((Session.current.xml_data.spirit.to_f / Session.current.xml_data.max_spirit.to_f) * 100).to_i
  else
      ((Session.current.xml_data.spirit.to_f / Session.current.xml_data.max_spirit.to_f) * 100).to_i >= num.to_i
  end
end

def checkstamina(num=nil)
  if num.nil?
      Session.current.xml_data.stamina
  else
      Session.current.xml_data.stamina >= num.to_i
  end
end

def maxstamina()
  Session.current.xml_data.max_stamina
end

def percentstamina(num=nil)
  if Session.current.xml_data.max_stamina == 0
      percent = 100
  else
      percent = ((Session.current.xml_data.stamina.to_f / Session.current.xml_data.max_stamina.to_f) * 100).to_i
  end
  if num.nil?
      percent
  else
...
def checkstance(num=nil)
  if num.nil?
      Session.current.xml_data.stance_text
  elsif (num.class == String) and (num.to_i == 0)
      if num =~ /off/i
        Session.current.xml_data.stance_value == 0
      elsif num =~ /adv/i
        Session.current.xml_data.stance_value.between?(01, 20)
      elsif num =~ /for/i
        Session.current.xml_data.stance_value.between?(21, 40)
      elsif num =~ /neu/i
        Session.current.xml_data.stance_value.between?(41, 60)
      elsif num =~ /gua/i
        Session.current.xml_data.stance_value.between?(61, 80)
      elsif num =~ /def/i
        Session.current.xml_data.stance_value == 100
      else
      ...
      Session.current.xml_data.stance_value == num.to_i
  else
      echo "checkstance: invalid argument (#{num}).  Must be off/adv/for/neu/gua/def or 0-100"
      nil
  end
...
def percentstance(num=nil)
  if num.nil?
      Session.current.xml_data.stance_value
  else
      Session.current.xml_data.stance_value >= num.to_i
  end
end
...
def checkencumbrance(string=nil)
  if string.nil?
      Session.current.xml_data.encumbrance_text
  elsif (string.class == Fixnum) or (string =~ /^[0-9]+$/ and string = string.to_i)
      string <= Session.current.xml_data.encumbrance_value
  else
      # fixme
      if string =~ /#{Session.current.xml_data.encumbrance_text}/i
        true
      else
        false
      end
...
def percentencumbrance(num=nil)
  if num.nil?
      Session.current.xml_data.encumbrance_value
  else
      num.to_i <= Session.current.xml_data.encumbrance_value
  end
end
...
def checkarea(*strings)
  strings.flatten!
  if strings.empty?
      Session.current.xml_data.room_title.split(',').first.sub('[','')
  else
      Session.current.xml_data.room_title.split(',').first =~ /#{strings.join('|')}/i
  end
end
...
def checkroom(*strings)
  strings.flatten!
  if strings.empty?
      Session.current.xml_data.room_title.chomp
  else
      Session.current.xml_data.room_title =~ /#{strings.join('|')}/i
  end
end

def outside?
  if Session.current.xml_data.room_exits_string =~ /Obvious paths:/
      true
  else
      false
  end
...
def checkfamarea(*strings)
  strings.flatten!
  if strings.empty? then return Session.current.xml_data.familiar_room_title.split(',').first.sub('[','') end
  Session.current.xml_data.familiar_room_title.split(',').first =~ /#{strings.join('|')}/i
end

def checkfampaths(dir="none")
  if dir == "none"
      if Session.current.xml_data.familiar_room_exits.empty?
        return false
      else
        return Session.current.xml_data.familiar_room_exits
      end
  else
      Session.current.xml_data.familiar_room_exits.include?(dir)
  end
end

def checkfamroom(*strings)
  strings.flatten! ; if strings.empty? then return Session.current.xml_data.familiar_room_title.chomp end
  Session.current.xml_data.familiar_room_title =~ /#{strings.join('|')}/i
end

def checkfamnpcs(*strings)
  parsed = Array.new
  Session.current.xml_data.familiar_npcs.each { |val| parsed.push(val.split.last) }
  if strings.empty?
      if parsed.empty?
        return false
      else
...
def checkfampcs(*strings)
  ...
  Session.current.xml_data.familiar_pcs.to_s.gsub(/Lord |Lady |Great |High |Renowned |Grand |Apprentice |Novice |Journeyman /,'').split(',').each { |line| familiar_pcs.push(line.slice(/[A-Z][a-z]+/)) }
...
def checkroomdescrip(*val)
  val.flatten!
  if val.empty?
      return Session.current.xml_data.room_description
  else
      return Session.current.xml_data.room_description =~ /#{val.join('|')}/i
  end
end
...
def checkfamroomdescrip(*val)
  val.flatten!
  if val.empty?
      return Session.current.xml_data.familiar_room_description
  else
      return Session.current.xml_data.familiar_room_description =~ /#{val.join('|')}/i
  end
end
...
def checkbounty
  if Session.current.xml_data.bounty_task
      return Session.current.xml_data.bounty_task
  else
      return nil
  end
end
...
def reget(*lines)
  ...
  session = Script.current.session
  return if session.nil?
  lines.flatten!
  if caller.find { |c| c =~ /regetall/ }
      history = (session.server_buffer.history + session.server_buffer).join("\n")
  else
      history = session.server_buffer.dup.join("\n")
  end
...
def put(*messages)
  Script.current
  messages.each { |message| Script.current.session.puts(message) }
end
...
def respond(*messages)
  _respond(*messages.map {|thing|
    line = Cabal::Escape.to_front_end(thing.inspect)
    line = line.slice(1..-2) if line[0] == %["] && line[-1] == %["]
    line
  })
end
...
def sf_to_wiz(line, session)
  begin
    ...
    if line =~ /<LaunchURL src="(.*?)" \/>/
      session.client_sock.puts "\034GSw00005\r\nhttps://www.play.net#{$1}\r\n"
    end
    ...
    session.client_sock.puts "--- Error: sf_to_wiz: #{$!}"
    session.client_sock.puts 'line=%s ' % line.to_s
  end
end
