    ...
    begin
      ...
      if client_string =~ /^(?:<c>)?#{$lich_char}(.+)$/
        ...
        if cmd =~ /^k$|^kill$|^stop$/
            ...
            if script.nil?
              session.to_client "--- Lich: #{target} does not appear to be running! Use ';list' or ';listall' to see what's active."
            ...
            if list.empty?
              ...
              session.to_client "--- Lich: #{list.collect { |s| s.paused? ? "#{s.name} (paused)" : s.name }.join(", ")}"
            end
            list = nil
        elsif cmd =~ /^force\s+[^\s]+/
            if cmd =~ /^force\s+([^\s]+)\s+(.+)$/
          ...
          if cmd.split[1] == "to"
            ...
            if script
                ...
                session.to_client "--- sent to '#{script.name}': #{msg}"
            else
                session.to_client "--- Lich: '#{cmd.split[2].chomp.strip}' does not match any active script!"
            end
            script = nil
          else
